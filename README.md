...
![自主看盘台 V2 页面](outputs/dashboard-v5.png)
...
- 自动区分阴线、阳线和十字线，识别长实体、锤头、长上影、阳包阴与阴包阳，并显示实体和影线判断口径。
- 根据 MA5/20/60 排列、MA20 五日斜率和收盘位置解释上升、下降、震荡、可能转强或可能转弱状态。
- 转折确认把候选形态、等待确认、后续收盘越过形态边界和候选失效分开；同时检查确认方向是否与均线结构一致，并明确价格确认不等于趋势永久反转。
- 价格阈值、均线穿越、区间突破、成交量放大、RSI 区域、MACD 金叉/死叉、突破并放量、趋势状态、K线形态九类规则。
- 技术状态解释区同时展示支持证据、冲突证据、20日关键位置、后续确认条件和观察清单；不输出买卖指令。
...
- 公司与市场背景区展示上证、深证、创业板指数、沪深上涨/下跌/平盘家数和上涨占比，以及所选股票的公开市值和估值字段；东方财富公司概况失败时降级到腾讯的部分字段。
- 市场判断优先使用真实沪深涨跌家数；宽度不可用时才退回主要指数快照，并明确标出当前范围不含北交所、数据来源和缓存状态。
- 服务端后台预警循环；浏览器关闭后仍检查并保存触发记录。
- Windows 原生托盘与通知；可打开页面、暂停/恢复预警、查看日志和安全退出。托盘不可用时自动降级为浏览器模式，服务和后台预警不会因此退出。
- 数据源健康区提供一键通知测试：EXE 模式报告 Windows Shell 是否接受请求，源码或无托盘模式报告浏览器降级与权限状态。
- 无控制台便携 EXE、同端口单实例保护、8765–8775 自动端口回退和本地轮转日志。
- 数据源自动降级、字段校验、最近成功/错误/耗时和实际来源展示。
- 数据源健康区可单独检测 HiThink 官方认证、行情快照和历史日 K，并区分未配置、无权限、限流、服务异常和契约异常；诊断结果不返回 Key。
...
![所有实时来源失败时的缓存降级页面](outputs/cache-fallback.png)
...
HiThink 当前官方文档规定：请求使用 `X-api-key`，成功条件是响应 `code == 0`，快照和历史记录位于 `data.item`。Key 只从 `HITHINK_FINANCE_API_KEY` 环境变量读取，不进入代码、前端、日志、测试样本或 Git。调用方错误 `1xxx/2xxx/3xxx` 直接报告；限流 `4001` 和服务异常 `5001/5002/5003` 最多退避重试 3 次，之后再由公开源接管。
...
- **阴阳线与形态**：收盘高于开盘为阳线，低于为阴线；实体不超过当日振幅 10% 时标为十字线。长实体、上下影、吞没等形态均使用固定比例与实体覆盖关系判断。
- **趋势状态**：上升或下降结构要求 MA5、MA20、MA60 同向排列、收盘位于 MA20 相应一侧，且 MA20 最近 5 个交易日斜率同向；条件未完全同步时标为可能转强、可能转弱或震荡。
...
- 趋势状态：当已完成日 K 首次进入指定趋势状态时触发；连续保持同一状态不会重复触发。
- K线形态：按最新已完成日 K 的实体、影线和前一根实体关系判断；形态只表示当日价格结构，不等于反转已经确认。
...
- `GET /api/history?symbol=600519.SH&limit=250`：日 K、全部指标，以及趋势、K线形态、证据、关键位置和观察步骤组成的 `analysis`。
- `GET /api/company?symbol=600519.SH`、`GET /api/market-overview`：公开公司背景、三大指数及沪深涨跌家数，指数与宽度分别报告实时、缓存或不可用状态。
...
- `POST /api/hithink-test`：只检测 HiThink 官方快照与日 K，不经过公开源回退，也不返回 Key。
- `POST /api/notification-test`：测试当前 Windows 原生通知或浏览器降级通道，不生成预警记录。
...
python -m py_compile src\server.py src\providers.py src\indicators.py src\analysis.py src\windows_desktop.py
```

截至 2026-09-21，本地自动化测试为 73 项，覆盖后台监控、原生通知与一键诊断、托盘命令与失败降级、便携模式注册表保护、Windows 独占端口与自动回退、绝对路径防泄漏、并发状态保护、状态迁移与备份轮换、导入校验、MA/RSI/MACD、趋势/K线形态/转折确认和九类规则、沪深市场宽度解析与优先判断、HiThink 契约解析/错误分级/退避重试/无 Key 诊断、无未来数据回放、前瞻收益样本计算、交易时段跳过、预警审计、跨午夜安静时段、记录筛选/CSV、防公式注入、字段异常、公司信息降级、来源回退、缓存和官方休市日历。当天实测证据、未验证项和外部来源影响见 [`outputs/验证记录.md`](outputs/验证记录.md)。
